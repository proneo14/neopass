...
export function HealthReport() {
  ...
  const [masterPwVisible, setMasterPwVisible] = useState(false);
        ...
        {showMasterPwCheck && (
          <div className="mt-3 space-y-2">
            <div className="relative">
              <input
                type={masterPwVisible ? 'text' : 'password'}
                placeholder="Type your master password to check"
                value={masterPwCheck}
                onChange={(e) => setMasterPwCheck(e.target.value)}
                className="w-full px-3 py-2 pr-10 rounded-md bg-surface-800 border border-surface-600 text-surface-100 text-sm placeholder-surface-500 focus:outline-none focus:ring-2 focus:ring-accent-500"
                autoFocus
              />
              <button
                type="button"
                onClick={() => setMasterPwVisible(!masterPwVisible)}
            ...
            <button
              onClick={() => { setShowMasterPwCheck(false); setMasterPwCheck(''); setMasterPwVisible(false); }}
